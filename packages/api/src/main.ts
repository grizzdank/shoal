...
async function bootstrap() {
  ...
  app
    .getHttpAdapter()
    .get('/health', (_req: unknown, res: { json: (body: object) => void }) => {
      res.json({ status: 'ok' });
    });
  ...
  http.post(
    ...
    async (
      ...
      try {
        ...
        const actorId =
          typeof req.body.actorId === 'string' ? req.body.actorId : 'agent';
        const result = await evaluateMessageContent({
          text,
          direction: 'inbound',
          actorId,
        });
        ...
        res
          .status(500)
          .json({
            error: 'governance_message_received_failed',
            detail: String(error),
          });
      }
    },
    ...
    async (
      ...
      try {
        ...
        res
          .status(500)
          .json({
            error: 'governance_message_sending_failed',
            detail: String(error),
          });
      }
    },
    ...
    async (
      ...
      try {
        ...
        res
          .status(500)
          .json({
            error: 'governance_before_tool_call_failed',
            detail: String(error),
          });
      }
    },
  ...
  http.post(
    ...
    async (
      ...
      try {
        ...
        const costTokens =
          typeof req.body.costTokens === 'number' &&
          Number.isFinite(req.body.costTokens)
        ...
        res
          .status(500)
          .json({
            error: 'governance_tool_result_failed',
            detail: String(error),
          });
      }
    },
