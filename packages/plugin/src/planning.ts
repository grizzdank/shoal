...
export function formatConstraintsForPrompt(c: ConstraintExpression): string {
  ...
  if (c.forbiddenTools?.length)
    lines.push(`Restricted tools: ${c.forbiddenTools.join(', ')}`);
  if (c.allowedTools?.length)
    lines.push(`Permitted tools: ${c.allowedTools.join(', ')}`);
  if (c.requiresApproval?.length)
    lines.push(`Requires approval: ${c.requiresApproval.join(', ')}`);
